...
git clone --recurse-submodules git@github.com:ngstep/yellowbox.git "$CHROOT_DIR"/yellowbox
systemd-nspawn -D "$CHROOT_DIR" \
    bash -c "cd yellowbox && make install"

# Install Application Wrappers
cp -R overlays/Applications/* "$CHROOT_DIR/Applications/"
...
mksquashfs "$CHROOT_DIR" "$SQUASHFS_IMG" -b 1M -comp xz -Xdict-size 100% -noappend -always-use-fragments
